...
export function generateMicroPythonFirmware(doc: MHDLDocument): BuildArtifact[] {
  ...
  // Medical safety: import hardware watchdog timer
  if (doc.meta?.medical) {
    lines.push(`from machine import WDT`);
  }
  lines.push(``);
  ...
  if (doc.meta?.medical) {
    ...
    lines.push(`    # If the main loop stops calling wdt.feed(), the ESP32 will`);
    lines.push(`    # automatically reboot, preventing indefinite unresponsive states.`);
    lines.push(`    wdt = WDT(timeout=30000)  # 30 second watchdog`);
  }
  ...
  // Medical safety: feed the watchdog at the end of each loop iteration
  if (doc.meta?.medical) {
    lines.push(``);
    lines.push(`        wdt.feed()  # Feed the watchdog — must be called every <30s`);
  }
